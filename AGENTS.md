...
- `Backend/` — Laravel 13 API. Since 2026 the master data module (Kategori, Sub Kategori, Merk, Satuan, Gudang, Rak, Bin, Supplier, Customer, Vendor, Barang, Departemen, Proyek, Work Order, User, Role) is API-backed behind Sanctum bearer-token auth + `role.access` RBAC middleware; the Persediaan module also shipped (`item_stock` + `stock_movements`/`stock_documents`, `GET /api/persediaan/stock`, `/stock-card`, `/stock-minimum`, `/valuation`, `/stock-documents` + `POST /stock-documents` — the latter is what the Transaksi → Barang Masuk form posts to). `Backend/AGENTS.md` holds the API conventions, schema notes, and env setup.
...
UI-only WMS demo — deterministic dummy data from `src/lib/wms-data.ts` (seeded PRNG), except the API-backed master pages, the Persediaan pages `persediaan/stock` (Stock Saat Ini), `persediaan/kartu-stock` (Kartu Stock), `persediaan/mutasi` (Mutasi Stock), `persediaan/stock-minimum` (Stock Minimum), `persediaan/nilai` (Nilai Persediaan), `persediaan/adjustment` (Adjustment), and the Transaksi pages `transaksi/masuk` (Barang Masuk), `transaksi/keluar` (Barang Keluar), `transaksi/transfer` (Transfer Gudang), `transaksi/retur-pembelian` (Retur Pembelian), and `transaksi/retur-penjualan` (Retur Penjualan), and the Pengadaan page `pengadaan/receive-goods` (Receive Goods — list = Penerimaan ber-referensi PO, form = `BarangMasukForm` variant Pengadaan dengan No. PO wajib) — these post (Draft/Selesai) to `POST /api/persediaan/stock-documents` (`Penerimaan`/`Pengeluaran`/`Transfer Gudang`/`Retur Pembelian`/`Retur Penjualan`, nomor `BM`/`BK`/`TF`/`RP`/`RJ`/`YYYY/#####`; Retur Pembelian = OUT ke supplier dengan referensi wajib dokumen Barang Masuk sumber (`source_document_id`/`source_line_id`, harga beli asal dari baris Penerimaan, cap sisa qty di server), Retur Penjualan = IN dari customer dengan referensi dokumen Barang Keluar sumber (sama: `source_document_id`/`source_line_id`, harga & bin kembali dari baris Pengeluaran, cap sisa qty di server), alasan retur masuk ke `note`), so they need `Persediaan` Tulis even though the menu lives under Transaksi. Transfer Gudang mengirim `destination_warehouse_id` (gudang tujuan) dan memosting pasangan OUT+IN sekaligus. Menyelesaikan Stock Opname membuat koreksi Stock Adjustment **Draft** (ter-link opname via `source_document_id`) yang ditinjau lalu diposting dari halaman `persediaan/adjustment` (bukan posting otomatis). Real Sanctum login: seeded test accounts (password dari `DEMO_PASSWORD` di `Backend/.env` — lihat `Frontend/docs/akun-login.md`); Administrator is `USR-001 Rudi Hartono` (`rudi.hartono@kelolagudang.id`). Managed by **bun** (both `bun.lock` and `package-lock.json` must stay in sync). No typecheck script — use `npx tsc --noEmit`; frontend unit tests run via `npm test` (vitest run). The real product requirements are in `Frontend/README.md`. Full details in `Frontend/AGENTS.md`.

## Backend (Laravel 13)

PostgreSQL 18 at `127.0.0.1:5432` (user `postgres`, password `postgres`), **NOT on PATH** — use full path `C:/Program Files/PostgreSQL/18/bin/psql.exe` (`createdb.exe`). Dev DB `kelolagudang`, test DB `kelolagudang_test` (per `phpunit.xml`). The Persediaan module normalized stock into `item_stock` (composite PK `item_id, warehouse_id, bin_id`) plus `stock_movements` and `stock_documents`/`stock_document_lines` (Mutasi Stock — posting/cancel via `StockDocumentService`); `items.stock`/`reserved` are still denormalized carryovers. Master data includes `racks`/`bins` (Rak / Bin Location), `suppliers`/`customers`/`vendors` (Supplier / Customer / Vendor), and `departments`/`projects`/`work_orders`; `items.default_rack_id`/`default_bin_id`/`preferred_supplier_id` are FKs (`nullOnDelete`). Full commands, API conventions, and schema notes in `Backend/AGENTS.md`.

### Auth & RBAC (since 2026)

Real Sanctum **bearer-token** auth — the API is NOT open anymore. `POST /api/auth/login` (throttled 5/min) returns `{ data, access, token }`; `GET /api/auth/me` and `POST /api/auth/logout` (deletes the current token) require `auth:sanctum`. `/api/master/*` requires `auth:sanctum` + `role.access:Master Data` and `/api/persediaan/*` requires `auth:sanctum` + `role.access:Persediaan` (`EnsureRoleAccess`, level per HTTP verb: GET/HEAD=Baca, POST/PUT/PATCH=Tulis, DELETE=Kelola). `role_permissions` maps role→`{module, level}` and is editable via `PUT /api/master/roles/{role}`. The frontend stores the token in localStorage (`kg-token`) and sends `Authorization: Bearer <token>` via `src/lib/api.ts`; `useAuth` gates the `/login` redirect on token presence and the sidebar/route gating mirrors `access` (`hasModule`/`hasModuleLevel` — see `Frontend/AGENTS.md`). No session cookies, no `/sanctum/csrf-cookie`, no `X-XSRF-TOKEN`. Do not regress to a fully open API.
