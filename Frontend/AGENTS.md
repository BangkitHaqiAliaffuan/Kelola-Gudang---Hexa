...
- Tests: `npm test` (`vitest run`, 5 specs, 52 tests: `src/routes/index.spec.tsx`, `src/lib/wms-data.spec.ts`, `barcode-label.spec.ts`, `stock-document-search.spec.ts`, `use-warehouse-filter.spec.ts`). Playwright (`playwright.config.ts`, auto-starts both servers) has only `e2e/auth.setup.ts` (needs `DEMO_PASSWORD` env from `Backend/.env`) + read-only GET-only `e2e/screenshots/read-only.spec.ts`. No CI, no typecheck script — use `npx tsc --noEmit`.
