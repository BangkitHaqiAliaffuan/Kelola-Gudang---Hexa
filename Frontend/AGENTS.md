...
- **Auth is bearer-token based** (no session cookies, no `/sanctum/csrf-cookie`, no `X-XSRF-TOKEN`): `POST /api/auth/login` returns `{ data, access, token }`; the token is stored in localStorage (`kg-token`, `src/lib/api.ts` `setAuthToken`) and sent as `Authorization: Bearer <token>`. Hooks in `src/hooks/use-auth.tsx` (boot via `me()`, gate on token presence), login page in `src/routes/login.tsx`, logout clears the token locally. Enforcement is server-side (`auth:sanctum` + `role.access` on `/api/master/*`); the frontend only uses `access` for UX mirroring.
