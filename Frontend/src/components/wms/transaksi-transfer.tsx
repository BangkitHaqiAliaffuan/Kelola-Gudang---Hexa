...
const transferRouteApi = getRouteApi("/transaksi/transfer");
...
export function TransferGudangPage() {
  ...
  const { data, isLoading, error, refetch } = useStockDocuments({ type: "Transfer Gudang" });
  ...
  const search = transferRouteApi.useSearch();
  const navigateDoc = transferRouteApi.useNavigate();
