...
import { Download, Search, TriangleAlert, PackageX, Truck } from "lucide-react";
...
export const Route = createFileRoute("/persediaan/stock-minimum")({
  head: () => ({
    meta: [
      { title: "Stock Minimum — KelolaGudang" },
      {
        name: "description",
        content: "Barang dengan stok di bawah minimum beserta usulan restock.",
      },
      { property: "og:title", content: "Stock Minimum — KelolaGudang" },
      {
        property: "og:description",
        content: "Pemakaian harian, hari sisa stok, dan kebutuhan restock per barang.",
      },
    ],
  }),
  component: StockMinimum,
});
...
function StockMinimum() {
  ...
  const handleExport = () => {
    ...
    downloadCsv(`stock-minimum-${new Date().toISOString().slice(0, 10)}.csv`, content);
  ...
  return (
    <>
      ...
      <Panel title="Daftar Stock Minimum" description={`${formatNumber(rows.length)} barang`}>
        <DataTable
          columns={columns}
          rows={rows}
          pageSize={12}
