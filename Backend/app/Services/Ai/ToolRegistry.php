...
final class ToolRegistry
{
    ...
    public static function all(): array
    {
        $tools = [
            ...
            new AiTool(
                name: 'analisis_data',
                description: 'Analitik lanjutan: ubah pertanyaan menjadi query SQL SELECT (baca saja) atas skema WMS untuk agregasi/tren. Contoh: total nilai stok per gudang, tren barang keluar per bulan, 10 barang paling sering keluar. Gunakan NAMA KOLOM BAHASA INGGRIS sesuai skema (mis. items.name, items.stock, items.min_stock — BUKAN `nama`/`stok`). JANGAN query information_schema/pg_catalog (akan ditolak). Hasil OTOMATIS dibatasi ke gudang yang boleh diakses user (jangan tambah filter gudang sendiri); untuk user terbatas, gunakan SELECT tunggal tanpa WITH/UNION/subquery. Filter tahun pakai EXTRACT(YEAR FROM document_date) = 2026 atau rentang document_date >= ... AND ... (PostgreSQL TIDAK punya fungsi YEAR(); kolom document_date BOLEH dipakai). "Nilai transaksi" bukan kolom — hitung SUM(l.qty * l.unit_cost) untuk barang masuk / SUM(l.qty * l.unit_price) untuk barang keluar via JOIN stock_document_lines l ON l.document_id = d.id lalu GROUP BY barang. DILARANG query eksplorasi skema (SELECT * LIMIT 1, cek MIN/MAX tanggal, tebak kolom): skema di system prompt sudah lengkap dan terpercaya — jawab LANGSUNG dengan SATU query final.',
                schema: [
                    'type' => 'object',
                    'properties' => [
