...
Laravel 13 API (`/api`) powering the master data module (Kategori, Sub Kategori, Merk, Satuan, Gudang, Rak, Bin, Supplier, Customer, Vendor, Barang). The frontend (`Frontend/`) calls this API for those pages; the rest of the app is still UI-only dummy data.
...
- Deleting a category that still has sub-categories/items, a rack with bins/items, a bin with items, a warehouse with racks/items, or a supplier still referenced by items is blocked (422).
