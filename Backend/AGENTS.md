...
- Supplier/Customer/Vendor carry "detail PT" fields: `legal_name`, `nib` (13 digits), `npwp` (validated via `App\Rules\ValidNpwp`, backed by `App\Support\Npwp` — Luhn checksum on the leading digits, see `tests/Unit/NpwpTest.php`), `pic_name`, `website`, bank info (`bank_name`/`bank_account_no`/`bank_account_name`), and a verification workflow (`verification_status` default `unverified`, `Rule::in(['unverified','verified','rejected'])`, plus nullable `verification_note` and `verified_at`). `verified_at` is set to `now()` when a status becomes `verified`, cleared when it isn't.
