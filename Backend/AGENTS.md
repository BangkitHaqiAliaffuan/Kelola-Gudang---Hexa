...
- **`GET /api/master/roles`** (`RoleController@index`) is a read-only catalog of the 4 user roles derived from `StoreUserRequest::ROLES`, each with `id` (1–4, enum order), `user_count` (total users with that role), and `active_user_count` (where `is_active` is true). No `roles` table/CRUD yet — roles are a validated string enum on `users.role`; RBAC (roles + permissions + gate) ships with auth later. The "Hak Akses" module→level mapping lives client-side in `Frontend/src/lib/schemas.ts` (`ROLE_ACCESS`).
